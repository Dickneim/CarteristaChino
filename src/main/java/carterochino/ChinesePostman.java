...
public class ChinesePostman {
    ...
    // ✅ Verificamos si ya es Euleriano
    if (ODD.isEmpty()) {
        grafo_copia = grafo;
        if (areaPasos != null) {
            areaPasos.append("El grafo ya es Euleriano. No se requiere duplicar aristas.\n");
        }
    } else {
        // 🔄 Paso 2: clonar aristas
        ...
        // 🔢 Paso 3: distancias mínimas entre vértices de grado impar
        ...
        // 🔧 Paso 4: encontrar el mejor emparejamiento
        ...
        // ➕ Paso 5: duplicar caminos mínimos
        for (List<Integer> par : mejorMatching) {
            ...
            for (int i = camino.size() - 1; i > 0; i--) {
                ...
                if (original != null) {
                    ...
                    grafo_copia.agregarArista(destino, origen, original.getPeso()); // si es no dirigido
                }
            }
        }
    }
    ...
    if (areaPasos != null) {
        for (Vertice v : grafo_copia.getVertice()) {
            areaPasos.append("Grado final del vertice " + v.getId() + ": " + grafo_copia.grado(v.getId()) + "\n");
        }
    }

    // ▶ Ejecutar Fleury
    Fleury fleury = new Fleury();
    fleury.setTextArea(areaPasos);
    fleury.FleuryAlgorithm(grafo_copia);
}
